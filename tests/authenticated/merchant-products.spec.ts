...
function summaryCard(page: Page, title: string, count: number) {
  return page.getByRole('button', { name: new RegExp(`^${title}\\s+${count}\\b`) })
}

function productTab(page: Page, title: string, count: number) {
  return page.getByRole('tab', { name: new RegExp(`^${title}\\s+${count}$`) })
}

function productRow(page: Page): Locator {
...
async function assertEmptySeededCatalog(page: Page) {
...
async function assertProductRow(page: Page, stock: string, status: string) {
...
test('merchant creates a draft, edits stock, and submits the product for review', async ({
  ...
  await page
    .getByRole('button', { name: 'Save & submit for review', exact: true })
    .click()
